...
const useAxiosPrivate = () => {
  ...
  const axiosInstance = axios.create({
    baseURL: 'https://api.atlassian.com/', // Replace with your actual base URL
  ...
  const fetchData = async ({ oldUrl, method, data = {}, params }) => {
    setLoading(true);

    let url = oldUrl;

    if (url !== 'oauth/token/accessible-resources') {
      url = `/ex/jira/${resource[0]?.id}/rest/api/3/${oldUrl}`;
    }
